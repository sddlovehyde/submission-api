...
def unzip_and_get_info():
  ...
  if submissionPackName:
     ...
     if (os.path.isfile(submissionPack[0] + 'Pega_Report/CTS/' + CTS_report[0] + '/device-info-files/PropertyDeviceInfo.deviceinfo.json') == True):
        CTS_report_property = loadJsonFile(submissionPack[0] + 'Pega_Report/CTS/' + CTS_report[0] + '/device-info-files/PropertyDeviceInfo.deviceinfo.json')
        ...
        report_fingerprint = findElement(CTS_report_property["ro_property"], "name", "value", "ro.build.fingerprint")
        upload_companyID = findElement(APA_companyID_dict["customer"], "name", "value", report_clientID)
        
     elif (os.path.isfile(submissionPack[0] + 'Pega_Report/GTS/' + GTS_report[0] + '/device-info-files/PropertyDeviceInfo.deviceinfo.json') == True):
        print "CTS no device info, use GTS report to compare"
        GTS_report_property = loadJsonFile(submissionPack[0] + 'Pega_Report/GTS/' + GTS_report[0] + '/device-info-files/PropertyDeviceInfo.deviceinfo.json')
        APA_companyID_dict = loadJsonFile(cwd + '/cusomterID.json')
        report_clientID = findElement(GTS_report_property["ro_property"], "name", "value", "ro.com.google.clientidbase")
        report_fingerprint = findElement(GTS_report_property["ro_property"], "name", "value", "ro.build.fingerprint")
        upload_companyID = findElement(APA_companyID_dict["customer"], "name", "value", report_clientID)
...
def upload():
  header = CheckCredentials('973714329395-r1cjtidkf8jgutb4f2gno2dg9671av81.apps.googleusercontent.com', '_hKnrk2l6ilGgnie9D4n_lJt')
...
def main(argv):
  ...
  CTS_report_property = loadJsonFile(cwd + '/' + submissionPack[0] + '/Pega_Report/CTS/' + CTS_report[0] + '/device-info-files/PropertyDeviceInfo.deviceinfo.json')
  APA_companyID_dict = loadJsonFile(cwd + '/cusomterID.json')
  report_clientID = findElement(CTS_report_property["ro_property"], "name", "value", "ro.com.google.clientidbase")  
  upload_companyID = findElement(APA_companyID_dict["customer"], "name", "value", report_clientID)
